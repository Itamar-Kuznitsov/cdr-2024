...
import PyPDF2
#inculdes the try for the recent todo
...
class Obj:
    ...
    def get_parents(self):
        parent = re.search(b'Parent [0-9]+ [0-9]+ R', self.raw_data)
        if parent != None:
            return int(parent.string[parent.start():parent.end()].split()[1].decode())
        return -1

    def get_kids(self):
        parent = re.search(b'\/Kids.\[', self.raw_data)
        if parent != None:
            return parent.string[parent.start():parent.end()]
        return -1
    ...
    def remove_opt_from_id(self, id: int, opt: bytes):
        ...
        for i in range(offset, len(data)):
            ...
            if data[i] == ls[0] and brackets == 0 and arrows == 0 and i != offset:
                ...
                end_offset = i
                break
            elif data[i] == ls[3] and data[i+1] == ls[3]:
                arrows += 1
                cont = True
            elif data[i] == ls[4] and data[i+1] == ls[4]:
                arrows -= 1
                cont = True
...
class Trailer:
    ...
    def get_trailer_b(self):
        return b'trailer<</' + self.size + b'\r\n/' + self.root + b'\r\n/' + self.info + b'>>\r\n'

    def __repr__(self) -> str:
        return (b'trailer<</' + self.size + b'/' + self.root + b'/' + self.info + b'>>\r\n').decode()
...
class PdfFile:
    ...
    def create_xref(self):
        ...
        with open(self.file_name, 'rb') as f:
            ...
            if not xref_temp[0] == b'xref':
                ...
                if not fixed:
                    """
                    in case of lineraized pdfs
                    """
                    if b'obj' in xref[0:20]:
                        end = xref.find(b'endobj')
                        new_xref_str = xref[:end+6]
                        root = -1
                        size = -1
                        info = -1
                        parent = re.search(b'\/Root [0-9]* [0-9]* R', new_xref_str)
                        if parent != None:
        
                            root = parent.string[parent.start():parent.end()]
                        parent = re.search(b'\/Size [0-9]*', new_xref_str)
                        if parent != None:
                       
                            size = parent.string[parent.start():parent.end()]
                        parent = re.search(b'\/Info [0-9]* [0-9]* R', new_xref_str)
                        if parent != None:
                      
                            info = parent.string[parent.start():parent.end()]
                        self.trailer_obj.set_root(int(root.split(b' ')[1]))
                        self.trailer_obj.set_info(int(info.split(b' ')[1]))
                        self.trailer_obj.set_size(int(size.split(b' ')[1]))
                        """obj = xref.split(b' ')[0].decode()
                        obj_id = int(obj)
                        print(f'python3 ./pdf/pdf-parser.py -o {obj_id} -f {self.file_name}')
                        x = os.popen(f'python3 ./pdf/pdf-parser.py -o {obj_id} -f {self.file_name}').read()
                        print(x[x.find('>>')+2:].replace('\n','')[3:-1].split('\\x')[1:])"""
                        buff_size = 9
                        b_string_to_find = b'startxref'
                        offset = self.find_key_word(buff_size, b_string_to_find)
                        self.start_xref = offset
                        # open the PDF file and create a PdfFileReader object
                        with open(f'{self.file_name}', 'rb') as f:
                            reader = PyPDF2.PdfFileReader(f)
                            xref_temp = []
                            xref_temp.append(b'xref')
                            xref_temp.append('0 1'.encode())
                            xref_temp.append('0000000000 65535 f'.encode())
                            for obj_stream in reader.xref[0].items():
                                #id start_offset
                                xref_temp.append(f'{str(obj_stream[0])} 1'.encode())
                                ped ='0'*(10-len(str(obj_stream[1])))
                                xref_temp.append(f'{ped}{str(obj_stream[1])} 00000 n'.encode())
        ...
        try:
            ...
            with open(f'{self.file_name}', 'rb') as f:
                reader = PyPDF2.PdfFileReader(f)
                root_id = reader.trailer
                self.trailer_obj.set_size(root_id['/Size'])
                for entery in str(root_id).split(','):
                    if '/Root' in entery:
                        nums = ''
                        for i in entery:
                            if i.isnumeric():
                                nums += i
                        self.trailer_obj.set_root(int(nums))
                    if '/Info' in entery:
                        nums = ''
                        for i in entery:
                            if i.isnumeric():
                                nums += i
                        self.trailer_obj.set_info(int(nums))
                xref_temp = []
                xref_temp.append(b'xref')
                xref_temp.append(f'0 1'.encode())
                xref_temp.append(f'0000000000 65535 f'.encode())
                for obj_stream in reader.xref[0].items():
                    self.trailer_obj.set_size(len(obj_stream))
                    #id start_offset
                    xref_temp.append(f'{str(obj_stream[0])} 1'.encode())
                    ped ='0'*(10-len(str(obj_stream[1])))
                    xref_temp.append(f'{ped}{str(obj_stream[1])} 00000 n'.encode())
                ...
                b_string_to_find = b'xref'
                offset = self.find_key_word(4, b_string_to_find)
                self.start_xref = offset
    ...
    def create_trailer(self):
        try:
            ...
            trailer_str_tmp = b'trailer<< /Size 1 /Root 1 0 R /Info 1 0 R >>'
            self.trailer_str = trailer_str_tmp
            buff_size = 9
            b_string_to_find = b'startxref'
            offset = self.find_key_word(buff_size, b_string_to_find)
            self.start_trailer = offset
    ...
    def remove_obj_by_id(self, id: int, cdr_rule_dict):
        ...
        #print(get_t, self.trailer_obj.get_root(), id)
    ...
    def remove_unused(self):
        id_list = []
        for obj_id in self.xref_obj.obj_list_ref:
            id_list.append(obj_id[0])
        ...
        for i in id_list:
            cdr_rules_pdf = {"root": ['r', [b'/OpenAction', b'/JavaScript', b'/JS', b'/AcroForm', b'/JS', b'/AA',b'/URI']],
                            "/Font": ['c', [b'<</Type /Font\n/BaseFont /Helvetica\n/Subtype /Type1\n>>']],
                            "/Page":['r', [b'nothing here need removing']],
                            "default": ['ra']}
            with open(self.file_name,'rb') as f:
                parent = re.search( f"[^0-9]{i} 0 R".encode(),f.read())
                if parent != None:
                    pass
                else:
                    #print(i,)
                    self.remove_obj_by_id(i, cdr_rules_pdf)
    ...
    def dump_file(self,path, new_name = " "):
        ...
        with open(new_file_name, 'wb+') as nf:
            nf.write(raw_data)
        #print(self.body_obj.obj_dict[28])
        #print("refrash" + str(self.conut_re))
